<?php
  ...
  // L'utente ha effettuato l'accesso ed è autorizzato
    ...
    // Leggo la notifica
    $sql = "UPDATE notifiche SET letto = TRUE WHERE idUtente = '{$autenticazione -> id}'";
    $query = $mysqli -> query($sql);

    if(!$query) {
      $console -> alert('Impossibile eseguire la query di lettura delle notifiche. '.$mysqli -> error, $autenticazione -> id);
