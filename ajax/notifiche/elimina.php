<?php
  ...
  // L'utente ha effettuato l'accesso ed è autorizzato
    ...
    if(!preg_match("/^[0-9]+$/", $id))
      ...
      // Elimino la notifica
      $sql = "DELETE FROM notifiche WHERE id = '{$id}'";
      $query = $mysqli -> query($sql);

      if(!$query) {
        $console -> alert('Impossibile eseguire la query di eliminazione della notifica '.$mysqli -> error, $autenticazione -> id);
        stampaErrore("Impossibile eseguire la query di eliminazione della notifica!");
      }
