<?php
  ...
  $sql = "DELETE FROM socialNetworks WHERE idUtente = {$autenticazione -> id} AND tipo = 'facebook'";
  $query = $mysqli -> query($sql);

  if(!$query) {
    $console -> alert('Impossibile eliminare il collegamento con Facebook! '.$mysqli -> error, $autenticazione -> id);
    stampaErrore('Impossibile completare la richiesta!');
  }

  $notificheUtente = new Notifiche($mysqli, $autenticazione -> id);
  $notificheUtente -> link('Disconnessione da Facebook completata!', '/account/social.php');
