<?php
  ...
  $limGiorn = $mysqli -> real_escape_string(isset($_POST['limGiorn']) ? trim($_POST['limGiorn']) : '');
  $distanza = $mysqli -> real_escape_string(isset($_POST['distanza']) ? trim($_POST['distanza']) : '');
  ...
  // L'utente ha effettuato l'accesso ed è autorizzato
    ...
    if(!preg_match("/^[0-9]+$/", $distanza))
      stampaErrore('Distanza non valida!');

    if(!preg_match("/^[0-9]+$/", $limGiorn))
      stampaErrore('Limite non valido!');
    ...
    // Creo il Maker Space
    $sql = "UPDATE makerspace SET distanzaBeggiate = '{$distanza}', limiteBeggiateGiornaliere = '{$limGiorn}' WHERE id = '{$id}'";
    $query = $mysqli -> query($sql);

    if(!$query) {
      $console -> alert("Impossibile eseguire la query di aggiornamento delle impostazioni di presenza del Maker Space! ".$mysqli -> error, $autenticazione -> id);
      stampaErrore('Impossibile completare la richiesta!');
    }

    echo '{}';
    $console -> log("Aggiornate le impostazioni di presenza del Maker Space (ID: {$id}).", $autenticazione -> id);
  }
?>